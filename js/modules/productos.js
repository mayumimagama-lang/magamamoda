...
const ProductosModule = {
          ...
          <tbody>
            ${paged.length === 0 ? `
              ...
              paged.map(p => {
                ...
                const imgSrc   = p.imagen || p.imagen_url || '';
  ...
  App.toast('⏳ Comprimiendo y subiendo imagen (' + originalKB + 'KB)...', 'info');

  SupabaseStorage.subirImagen(file, 'productos').then(function(res) {
    const badge = document.getElementById('imgUploadBadge');
    ...
    if (res.ok) {
      ...
      if (wrap) {
        wrap.innerHTML =
          ...
          'background:rgba(22,163,74,0.85);color:#fff;font-size:11px;padding:3px 10px;' +
          'border-radius:20px;white-space:nowrap;">' +
          '<i class="fas fa-check" style="margin-right:4px;"></i>' + res.sizeKB + 'KB</div>';
      }
      App.toast('✅ Imagen subida: ' + originalKB + 'KB → ' + res.sizeKB + 'KB', 'success');
    } else {
      App.toast('❌ Error al subir imagen. Intenta de nuevo.', 'error');
    }
  });
},
