...
const VentasModule = {
  ...
  exportarVentas() {
    var filtered=this.getFiltered();
    var header='Fecha,Hora,Serie,Numero,Tipo,Cliente,Doc,Total,Estado,Metodo Pago,Cajero\n';
    var rows=filtered.map(function(v){
      var cli=(DB.clientes||[]).find(function(c){return c.id===v.cliente_id;});
      return v.fecha+','+v.hora+','+v.serie+','+v.numero+','+v.tipo+
        ',"'+(cli?cli.nombre:'')+'",'+(cli?cli.doc:'')+','+v.total.toFixed(2)+
        ','+v.estado+','+v.metodo_pago+','+(v.cajero||'');
    }).join('\n');
    var a=document.createElement('a');
    a.href=URL.createObjectURL(new Blob(['\uFEFF'+header+rows],{type:'text/csv;charset=utf-8;'}));
    a.download='ventas_'+this._fechaLocal()+'.csv'; a.click(); URL.revokeObjectURL(a.href);
