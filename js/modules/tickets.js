...
const CuentaCorrienteModule = {
    ...
    var toggle = function(id, label, valor) {
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--gray-100);">'+
        ...
        '<label style="position:relative;display:inline-block;width:44px;height:24px;cursor:pointer;">'+
          ...
          '<span style="position:absolute;inset:0;border-radius:24px;transition:0.3s;'+
            'background:'+(valor?'var(--accent)':'var(--gray-300)')+';">'+
        ...
        '<div style="display:flex;flex-direction:column;gap:16px;">'+
          ...
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-image" style="color:var(--accent);margin-right:5px;"></i>Logo de la Empresa'+
              '</div>'+
            '</div>'+
            '<div style="padding:18px 20px;display:flex;align-items:center;gap:20px;">'+
              ...
              '<div id="logoPreviewWrap" onclick="document.getElementById(\'cfg_logo_file\').click()" '+
                ...
                (cfg.logo
                  ...
                  : '<div style="text-align:center;color:var(--gray-400);pointer-events:none;">'+
                      '<i class="fas fa-building" style="font-size:36px;display:block;margin-bottom:8px;opacity:0.4;"></i>'+
                      '<span style="font-size:12px;font-weight:700;">Tu Logo</span>'+
                    '</div>'
                )+
              '</div>'+
              '<div style="flex:1;">'+
                '<input type="file" id="cfg_logo_file" accept="image/*" style="display:none;" onchange="CuentaCorrienteModule._subirLogo(this)"/>'+
                '<button onclick="document.getElementById(\'cfg_logo_file\').click()" class="btn btn-primary" style="width:100%;margin-bottom:8px;">'+
                  '<i class="fas fa-upload"></i> Subir Logo'+
                '</button>'+
                (cfg.logo
                  ? '<button onclick="CuentaCorrienteModule._quitarLogo()" class="btn btn-outline" style="width:100%;margin-bottom:8px;color:var(--danger);border-color:var(--danger);">'+
                    '<i class="fas fa-trash"></i> Quitar Logo</button>'
                  : ''
                )+
                '<p style="font-size:11px;color:var(--gray-400);line-height:1.5;">'+
                  'PNG, JPG o WebP · Máx. 2 MB<br/>'+
                  'Recomendado: fondo transparente (PNG), 300×300 px'+
                '</p>'+
              '</div>'+
            '</div>'+
          '</div>'+
          ...
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-store" style="color:var(--accent);margin-right:5px;"></i>Información de la Empresa'+
              '</div>'+
            '</div>'+
          ...
          // Card: Mensajes del ticket
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-comment" style="color:var(--accent);margin-right:5px;"></i>Mensajes y Pie de Ticket'+
              '</div>'+
            '</div>'+
            '<div style="padding:18px 20px;">'+
          ...
          // Card: Formato y diseño
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-ruler" style="color:var(--accent);margin-right:5px;"></i>Formato del Ticket'+
              '</div>'+
            '</div>'+
            '<div style="padding:18px 20px;">'+
              // Tamaño papel
              '<div style="margin-bottom:16px;">'+
                ...
                [
                  ...
                  var activo = cfg.tamano === t.val;
                  return '<button onclick="CuentaCorrienteModule.cfg.tamano=\''+t.val+'\';CuentaCorrienteModule._actualizarPreview();" '+
                    'id="tamano_btn_'+t.val+'" '+
                    'style="padding:12px 8px;border-radius:10px;border:2px solid '+(activo?'var(--accent)':'var(--gray-200)')+';'+
                    'background:'+(activo?'#eff6ff':'white')+';cursor:pointer;text-align:center;">'+
                    '<i class="fas '+t.icon+'" style="font-size:22px;color:'+(activo?'var(--accent)':'var(--gray-400)')+';display:block;margin-bottom:5px;"></i>'+
                    '<div style="font-size:13px;font-weight:800;color:'+(activo?'var(--accent)':'var(--gray-700)')+'">'+t.label+'</div>'+
              ...
              '<div style="margin-bottom:16px;">'+
                ...
                [
                  {val:'11',label:'Pequeño'},
                  {val:'12',label:'Normal'},
                  {val:'13',label:'Grande'},
                  {val:'14',label:'Muy grande'},
                ].map(function(f){
                  var activo = cfg.fuenteSize === f.val;
                  return '<button onclick="CuentaCorrienteModule.cfg.fuenteSize=\''+f.val+'\';CuentaCorrienteModule._actualizarPreview();" '+
                    'style="flex:1;padding:10px;border-radius:8px;border:2px solid '+(activo?'var(--accent)':'var(--gray-200)')+';'+
                    'background:'+(activo?'#eff6ff':'white')+';cursor:pointer;font-size:12px;font-weight:700;'+
                    'color:'+(activo?'var(--accent)':'var(--gray-600)')+';">'+f.label+'</button>';
                }).join('')+
                '</div>'+
              '</div>'+
              // Colores
              '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px;">'+
                '<div><label style="display:block;font-size:11px;font-weight:700;color:var(--gray-500);text-transform:uppercase;margin-bottom:6px;">COLOR ENCABEZADO</label>'+
                  '<div style="display:flex;align-items:center;gap:8px;">'+
                    '<input type="color" id="cfg_colorHeader" value="'+cfg.colorHeader+'" onchange="CuentaCorrienteModule._actualizarCfg()" '+
                      'style="width:44px;height:44px;border:none;border-radius:8px;cursor:pointer;"/>'+
                ...
                '<div><label style="display:block;font-size:11px;font-weight:700;color:var(--gray-500);text-transform:uppercase;margin-bottom:6px;">COLOR TOTAL</label>'+
                  '<div style="display:flex;align-items:center;gap:8px;">'+
                    '<input type="color" id="cfg_colorTotal" value="'+cfg.colorTotal+'" onchange="CuentaCorrienteModule._actualizarCfg()" '+
                      'style="width:44px;height:44px;border:none;border-radius:8px;cursor:pointer;"/>'+
          ...
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-eye" style="color:var(--accent);margin-right:5px;"></i>¿Qué mostrar en el ticket?'+
              '</div>'+
            '</div>'+
        ...
        '<div style="position:sticky;top:80px;">'+
          '<div class="card">'+
            '<div style="padding:14px 18px;border-bottom:1px solid var(--gray-200);display:flex;align-items:center;justify-content:space-between;">'+
              '<div style="font-size:12px;font-weight:800;color:var(--gray-400);text-transform:uppercase;letter-spacing:1px;">'+
                '<i class="fas fa-eye" style="color:var(--accent);margin-right:5px;"></i>Vista Previa del Ticket'+
              '</div>'+
              '<span style="font-size:11px;background:var(--accent);color:white;padding:2px 10px;border-radius:10px;">'+cfg.tamano+'</span>'+
            '</div>'+
            '<div id="ticketPreviewContainer" style="padding:16px;overflow-y:auto;max-height:700px;">'+
              preview+
            '</div>'+
            '<div style="padding:12px 18px;border-top:1px solid var(--gray-200);display:flex;gap:8px;">'+
              '<button onclick="CuentaCorrienteModule.guardarConfig()" '+
                'style="flex:1;padding:12px;background:linear-gradient(135deg,#15803d,#16a34a);color:white;'+
              ...
              '</button>'+
              '<button onclick="CuentaCorrienteModule._imprimirPreview()" '+
                'style="flex:1;padding:12px;background:var(--gray-100);color:var(--gray-600);'+
                'border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;">'+
                '<i class="fas fa-print" style="margin-right:6px;"></i>Imprimir prueba'+
              '</button>'+
            '</div>'+
          '</div>'+
        '</div>'+
      ...
      '<div style="max-width:'+ancho+';margin:0 auto;font-family:\'Courier New\',monospace;font-size:'+fs+'px;'+
        ...
        '<div style="background:'+hClr+';padding:16px 12px;text-align:center;color:white;">'+
          (cfg.mostrarLogo && cfg.logo
            ? '<img src="'+cfg.logo+'" style="max-width:80px;max-height:60px;object-fit:contain;display:block;margin:0 auto 8px;" alt="logo"/>'
            : ''
          )+
        ...
        '<div style="text-align:center;padding:10px 12px;border-top:1px dashed #ccc;background:#f8fafc;">'+
          ...
          (cfg.mostrarBarcode
            ? '<div style="margin-top:8px;"><i class="fas fa-barcode" style="font-size:32px;color:#333;"></i></div>'
            : ''
          )+
        '</div>'+
      '</div>';
  ...
  _actualizarCfg() {
    ...
    var campos = ['nombre','ruc','direccion','telefono','email','web','mensaje','mensaje2'];
    campos.forEach(function(k){ var el=get(k); if(el) cfg[k]=el.value; });
    var checks = ['mostrarLogo','mostrarRuc','mostrarDir','mostrarTel','mostrarEmail','mostrarWeb','mostrarIGV','mostrarBarcode'];
    checks.forEach(function(k){ var el=get(k); if(el) cfg[k]=el.checked; });
  ...
  _imprimirPreview() {
    ...
    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Vista Previa Ticket</title>'+
      '<style>body{font-family:Arial;background:#f0f0f0;display:flex;justify-content:center;padding:20px;}'+
      '.fas,.far{font-family:"Font Awesome 5 Free";font-weight:900;}</style>'+
  ...
  // ─────────────────────────────────────────
  // EDITAR TICKET (pantalla completa)
  // ─────────────────────────────────────────
    ...
    return (
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:13px 18px;background:var(--gray-50);border-radius:12px;border:1.5px solid var(--gray-200);margin-bottom:16px;">'+
        '<div style="display:flex;align-items:center;gap:12px;">'+
          '<button onclick="CuentaCorrienteModule.ticketEditando=null;App.renderPage();" style="background:white;color:var(--gray-700);border:1.5px solid var(--gray-200);border-radius:8px;padding:8px 16px;font-weight:700;cursor:pointer;font-size:13px;">'+
            '<i class="fas fa-arrow-left" style="margin-right:6px;"></i>Volver a Tickets</button>'+
          ...
          '</div>'+
        '</div>'+
        '<button onclick="CuentaCorrienteModule.guardarEdicion('+id+')" style="background:linear-gradient(135deg,#15803d,#16a34a);color:white;border:none;border-radius:8px;padding:10px 24px;font-weight:900;cursor:pointer;font-size:15px;box-shadow:0 4px 12px rgba(22,163,74,0.3);">'+
          '<i class="fas fa-save" style="margin-right:7px;"></i>GUARDAR CAMBIOS</button>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:1fr 360px;gap:16px;">'+
        '<div style="display:flex;flex-direction:column;gap:14px;">'+
          '<div class="card">'+
            '<div style="padding:14px 20px;border-bottom:1px solid var(--gray-200);"><div style="font-size:11px;font-weight:800;color:var(--gray-400);text-transform:uppercase;"><i class="fas fa-info-circle" style="color:var(--accent);margin-right:5px;"></i>Datos</div></div>'+
            '<div style="padding:14px 20px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;">'+
              '<div><div style="font-size:10px;font-weight:700;color:var(--gray-400);text-transform:uppercase;margin-bottom:5px;">SERIE</div><div style="padding:8px 12px;background:var(--gray-50);border:1.5px solid var(--gray-200);border-radius:8px;font-size:15px;font-weight:900;color:'+tClr+';">'+v.serie+'</div></div>'+
              '<div><div style="font-size:10px;font-weight:700;color:var(--gray-400);text-transform:uppercase;margin-bottom:5px;">NÚMERO</div><div style="padding:8px 12px;background:var(--gray-50);border:1.5px solid var(--gray-200);border-radius:8px;font-size:15px;font-weight:900;">'+v.numero+'</div></div>'+
        ...
        '<div style="display:flex;flex-direction:column;gap:14px;">'+
          ...
          '<div class="card">'+
            ...
            '<div style="padding:18px;">'+
              '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="font-size:16px;color:var(--gray-500);">Subtotal:</span><span style="font-size:20px;font-weight:900;color:var(--gray-800);">S/ '+subtotal.toFixed(2)+'</span></div>'+
              '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding-bottom:16px;border-bottom:2px solid var(--gray-200);"><span style="font-size:16px;color:var(--gray-500);">IGV (18%):</span><span style="font-size:20px;font-weight:900;color:var(--gray-800);">S/ '+igv.toFixed(2)+'</span></div>'+
